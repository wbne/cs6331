...
async function loadModel() {
	...
	const img = document.getElementById("blank");
	console.time('model');
	...
	let result = await model.executeAsync(zeroTensor);
	...
	// Infer the image
	const data = tf.expandDims(tf.browser.fromPixels(img));
	const height = data.shape[1];
	const width = data.shape[2];
	console.time('inference');
	result = await model.executeAsync(data);
	...
	// TODO: Add FC layers here to detect QR codes.
}

// this is ripped straight from the tfjs cocossd file. i need to study this ong
...
// i am once again ripping from the tfjs cocossd file
function buildDetectedObjects(
      width, height, boxes, scores,
      indexes, classes) {
...
//also ripped but this is more for debugging:
const CLASSES = {
  1: {
    name: '/m/01g317',
    id: 1,
